...
async function getTeams(){
  ...
  return parseTeamRows(summary,pcts,rt,pp,pk);
}
async function league(){
  const t=await getTeams(), vals=Object.values(t);
...
async function analyze(home,away){
  ...
  const [teams,l,fh,fa,bh,ba,gf]=await Promise.all([getTeams(),league(),getForm(home),getForm(away),getB2B(home),getB2B(away),getGoalieFactors()]);
  const h=teams[home],a=teams[away];
  if(!h||!a||!l.valid)throw new Error("Données équipes insuffisantes.");
  const x=expectedFrom(h,a,l,fh,fa,bh,ba,gf),m=markets(x.home,x.away);
  const [players,gh,ga]=await Promise.all([buildPlayers(home,away,x.home,x.away,x.shotsHome,x.shotsAway),projectGoalie((await getRoster(home)).goalies,home),projectGoalie((await getRoster(away)).goalies,away)]);
...
function renderAnalysis(d){
  ...
  const k=[["TOTAL BUTS",fmt(d.x.total)],["TOTAL TIRS",fmt(d.x.shotsHome+d.x.shotsAway)],["HOME OT",pct(d.m.home)],["AWAY OT",pct(d.m.away)],["OVER 5.5",pct(d.m.o55)],["BTTS",pct(d.m.btts)]];
...
async function runAnalysis(){
  ...
  $("loading").classList.remove("hidden");$("analysis").classList.add("hidden");
  try{renderAnalysis(await analyze(home,away));$("lastUpdate").textContent="Dernière analyse : "+new Date().toLocaleTimeString("fr-FR")}catch(e){$("emptyState").classList.remove("hidden");$("emptyState").innerHTML=`<div class="empty-icon">⚠️</div><h2>Analyse indisponible</h2><p>${e.message}</p>`}finally{$("loading").classList.add("hidden")}
}
...
function setup(){
  ...
  document.querySelectorAll(".nav-btn").forEach(b=>b.onclick=()=>{document.querySelectorAll(".nav-btn").forEach(x=>x.classList.remove("active"));b.classList.add("active");document.querySelectorAll(".view").forEach(x=>x.classList.remove("active"));$(`view-${b.dataset.view}`).classList.add("active");if(b.dataset.view==="matchs")schedule(0);if(b.dataset.view==="equipes")renderTeamTable()});
  ...
  $("refreshSchedule").onclick=()=>schedule(0);$("refreshTeams").onclick=renderTeamTable;
